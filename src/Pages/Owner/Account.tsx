...
import zaraLogo from '../../assets/owner/zara_logo.png'
import { useSelector } from 'react-redux'
import { Country, getCountries } from '../../Services/General/countries'
import { deleteCardUser, updateCardUser, updateUser } from '../../Services/Authentication/updateUser'
...
const Account: React.FC<OwnerAccountProps> = ({ setSelectedImageForNavbar }) => {
  ...
  if (!userDetails?.description || !userDetails?.companyName || !userDetails?.phone || !userDetails?.businessName || !userDetails?.VATNumber || !userDetails?.companyNumber || !userDetails?.websites) {
  ...
  const handleSubmit = async () => {
    try {
      if (!userDetails._id) {
        throw new Error("User details are not properly loaded.");
      }
      ...
      if (!userDetails?.description) {
        ...
        errorToast("Please enter a company name")
      } else if (!userDetails?.phone) {
        errorToast("Please enter a phone number")
      }
      else if (!userDetails?.landline) {
        errorToast("Please enter a landline number")
      } else if (!userDetails?.businessName) {
        errorToast("Please enter a contact name")
      } else if (!userDetails?.businessAddress) {
        errorToast("Please enter a business address")
      } else if (!userDetails?.VATNumber) {
        errorToast("Please enter a vat number")
      } else if (!userDetails?.companyNumber) {
        errorToast("Please enter a company number")
      } else if (!userDetails?.websites) {
        errorToast("Please enter a website")
      }
      else if (!userDetails?.city) {
        errorToast("Please enter a City or Town")
      }
  ...
  const handleSubmitCard = async (e: any) => {
    try {
      ...
      if (!state.bankName || !state.accountName || !state.sortCode || !state.accountNumber) {
        return errorToast("Please fill in all the required fields.");
      }
      ...
      if (!res.success) {
        ...
        const userData = await getUserData();
        setUserDetails(userData.result);
        return successToast(res.message)
  ...
  const { bankName, accountName, accountNumber, sortCode } = state;
  ...
  return (
    <div className='flex  footer-manage  font-modernEraBold'>
      <Toaster position='top-right' />
      <div className='hidden lg:block'>
        <OwnerSidebar />
      </div>
      <div className='w-[100%] lg:w-[95%] mx-auto z-10 p-4 lg:p-10 duration-500' style={{ fontFamily: "poppins, sans-serif" }}>
        <div className='w-[100%] m-auto'>
          ...
          <div className='bg-[#F9F0F0] p-2 lg:p-8 '>
            <div className='block lg:flex items-center gap-4'>
              <div className='flex items-center gap-4'>
                ...
                <input
                  ...
                  style={{ display: 'none' }}
                />
                <p onClick={() => fileInputRef.current?.click()} className='text-[#F66E6A] border-b-[1px] border-[#F66E6A] cursor-pointer'>Add profile picture</p>

              </div>


            </div>
            <input value={userDetails?.description} onChange={handleChange} className='hidden lg:block p-3 outline-none border-none w-[100%] lg:w-[95%] mt-8 
              ' type="text" placeholder='Describe your business briefly' name="description" id="" />
            <textarea value={userDetails?.description} onChange={handleChange} name="description" placeholder='Describe your business briefly' id="" rows={5} className='block lg:hidden p-2 mt-2 w-full' ></textarea>
            <br />
            <div className='hidden lg:block'>
              <div className=' flex items-center gap-2 w-[95%] mt-4'>
                <div className='w-[100%]'>
                  <h4 className='text-sm lg:text-md  font-medium tracking-wide'>Company Name</h4>
                  <input className='p-3 outline-none border-none w-[100%]' type="text" placeholder='Company Name' name="companyName" value={userDetails?.companyName} onChange={handleChange} id="" />
                </div>
                <div className='w-[100%]'>
                  <h4 className='text-sm lg:text-md  font-medium tracking-wide'>Phone Number</h4>
                  <div className='flex gap-1'>
                    <input className='p-3 outline-none rounded-md border-none w-[100%]' type="text" placeholder='Phone number' name="phone" value={userDetails?.phone} onChange={handleChange} id="" />
                  </div>
                </div>

                <div className='w-[100%]'>
                  <h4 className='text-sm lg:text-md  font-medium tracking-wide'>Landline Number</h4>
                  <div className='flex gap-1'>
                    <input className='p-3 outline-none rounded-md border-none w-[100%]' type="text" placeholder='Landline number' name="landline" value={userDetails?.landline} onChange={handleChange} id="" />
                  </div>
                </div>
              </div>
              <div className='grid lg:flex items-start gap-2 w-[100%] lg:w-[95%] mt-4 lg:mt-8'>
                <div className='w-[100%]'>
                  ...
                  <input className='p-3 rounded-md outline-none border-none w-[100%] text-gray-400' type="email" placeholder='raffly@gmail.com' name="email" value={userDetails?.email} id="" />
                </div>
                <div className='w-[100%]'>
                  <h4 className='text-md font-medium tracking-wide'>Password</h4>
                  <input className='p-3 rounded-md outline-none border-none w-[100%]' type="password" placeholder='Password' value="**************" name="" id="" />
                  <p onClick={handleSendOTP} className='mt-2 text-sm font-medium tracking-wide border-b-[1px] border-[#FF5F5F] w-fit text-[#FF5F5F] cursor-pointer'>Reset Password</p>
                </div>
              </div>
              <div className='flex items-center gap-2 w-[95%] mt-8'>
                <div className='w-[100%]'>
                  <h4 className='text-sm lg:text-md  font-medium tracking-wide'>Business Name</h4>
                  <input className='p-3 outline-none border-none w-[100%]' type="text" placeholder='John Smith' name="businessName" id="" onChange={handleChange} value={userDetails?.businessName} />
                </div>
                <div className='w-[100%]'>
                  <h4 className='text-sm lg:text-md  font-medium tracking-wide'>Business Address</h4>
                  <input className='p-3 outline-none border-none w-[100%]' type="text" placeholder='15 Av STR LONDON' name="businessAddress" id="" value={userDetails?.businessAddress} onChange={handleChange} />
                </div>
              ...
              <div className='flex items-center gap-2 w-[95%] mt-8'>
                <div className='w-[100%]'>
                  <h4 className='text-sm lg:text-md  font-medium tracking-wide'>City or Town</h4>
                  <input className='p-3 outline-none border-none w-[100%]' type="text" name="city" id="" onChange={handleChange} value={userDetails?.city} />
                </div>
                <div className='w-[100%]'>
                  <h4 className='text-sm lg:text-md  font-medium tracking-wide'>Postcode</h4>
                  <input className='p-3 outline-none border-none w-[100%]' type="text" placeholder='K1A 0B1' name="postcode" id="" value={userDetails?.postcode} onChange={handleChange} />
                </div>
                <div className='w-[100%]'>
                  <h4 className='text-sm lg:text-md  font-medium tracking-wide'>Country</h4>
                  <input className='p-3 outline-none border-none w-[100%]' type="text" placeholder='United Kingdom' name="country" id="" value={userDetails?.country} onChange={handleChange} />
                </div>

              </div>
              <div className='flex items-center gap-2 w-[95%] mt-8'>
                <div className='w-[100%]'>
                  <h4 className='text-sm lg:text-md  font-medium tracking-wide'>Vat Number</h4>
                  <input className='p-3 outline-none border-none w-[100%]' type="text" placeholder='56548821' name="VATNumber" onChange={handleChange} value={userDetails?.VATNumber} id="" />
                </div>
                <div className='w-[100%]'>
                  <h4 className='text-sm lg:text-md  font-medium tracking-wide'>Company Number</h4>
                  <input className='p-3 outline-none border-none w-[100%]' type="text" placeholder='+1597 5648 6644' onChange={handleChange} value={userDetails?.companyNumber} name="companyNumber" id="" />
                </div>
              </div>
            </div>
            <div className='block lg:hidden'>
              ...
              <br />
              <div className='w-[100%]'>
                <h4 className='text-sm lg:text-md  font-medium tracking-wide'>Email Address</h4>
                <input className='p-3 rounded-md outline-none border-none w-[100%] text-gray-400' type="email" placeholder='raffly@gmail.com' name="email" value={userDetails?.email} id="" />
              </div>
              <br />
              <div className='w-[100%]'>
                <h4 className='text-sm lg:text-md  font-medium tracking-wide'>Contact Name</h4>
                <input className='p-3 outline-none border-none w-[100%]' type="text" placeholder='John Smith' name="businessName" id="" onChange={handleChange} value={userDetails?.businessName} />
              </div>
              <br />
              <div className='w-[100%]'>
              ...
              <div className='flex items-center gap-2 w-[100%] mt-8'>
                ...
                <div className='w-[100%]'>
                  <h4 className='text-sm lg:text-md  font-medium tracking-wide'>Business Address</h4>
                  <input className='p-3 outline-none border-none w-[100%]' type="text" placeholder='15 Av STR LONDON' name="businessAddress" id="" value={userDetails?.businessAddress} />
              ...
              <div className='flex items-center gap-2 w-[95%] mt-8'>
                ...
                <div className='w-[100%]'>
                  <h4 className='text-sm lg:text-md  font-medium tracking-wide'>Company Number</h4>
                  <input className='p-3 outline-none border-none w-[100%]' type="text" placeholder='+1597 5648 6644' name="companyNumber" onChange={handleChange} value={userDetails?.companyNumber} id="" />
                </div>
              </div>
            </div>

            {/* <br /> */}
            <div className='block lg:flex items-start gap-2 w-[95%] mt-4'>
              <div className='w-[100%] mt-4 lg:mt-0'>
                <h4 className='text-sm lg:text-md  font-medium tracking-wide'>Website</h4>
                <input className='p-3 outline-none border-none w-[100%]' type="text" placeholder='www.zara.com' name="websites" onChange={handleChange} value={userDetails?.websites} id="" />

              </div>
            </div>
            <div className='h-[1px] bg-gray-400 mt-4'></div>
            <button onClick={handleSubmit} className='bg-[#FF6A78] w-[100%] text-white p-3 rounded-md mt-4 save-button'>
              Save Changes
            </button>
          </div>
        </div>
        ...
        <div className='w-[100%] m-auto'>
          <div className='bg-[#F9F0F0] p-2 lg:p-8 '>
            <h2 className='text-lg font-medium tracking-wider'>Bank Details</h2>
            ...
            <div className='gap-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 mt-6'>
              {
                userDetails?.wallet?.cardDetails.map((item, i) => {
                  return (
                    <div className='px-8 pb-8 rounded-2xl bg-white w-[400px] shadow-xl'>
                      <div className='flex items-center justify-end pt-4  gap-4'>
                        <svg onClick={() => { handleCardEdit(item, i) }} style={{ cursor: "pointer" }} width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M22.812 1.70787C21.9628 0.858404 20.9396 0.433594 19.7432 0.433594H5.26484C4.06848 0.433594 3.04528 0.858404 2.19566 1.70787C1.3462 2.55749 0.921387 3.58064 0.921387 4.77705V19.2553C0.921387 20.4516 1.3462 21.4748 2.19566 22.3244C3.04528 23.1741 4.06848 23.5989 5.26484 23.5989H19.7431C20.9394 23.5989 21.9626 23.1741 22.8119 22.3244C23.6615 21.4748 24.0863 20.4516 24.0863 19.2553V4.77705C24.0862 3.58064 23.6614 2.55733 22.812 1.70787ZM9.12564 19.7379H4.78208V15.3943L12.9865 7.1901L17.3297 11.5335L9.12564 19.7379ZM19.6827 9.18083L18.2954 10.5685L13.9518 6.2248L15.3391 4.83737C15.6207 4.55609 15.9624 4.41515 16.3648 4.41515C16.7668 4.41515 17.1088 4.55587 17.3901 4.83737L19.6826 7.12983C19.9639 7.41122 20.1049 7.75305 20.1049 8.15535C20.105 8.5575 19.9643 8.89928 19.6827 9.18083Z" fill="#FF5F5F" />
                          <path d="M6.22998 15.9975V16.8417H7.67778V18.2898H8.52223L9.30656 17.5055L7.01415 15.2129L6.22998 15.9975Z" fill="#FF5F5F" />
                          <path d="M8.41701 13.7657C8.24601 13.9363 8.23101 14.0871 8.37163 14.218C8.50232 14.359 8.65318 14.3438 8.82412 14.1728L13.2128 9.78403C13.3838 9.61341 13.3987 9.46255 13.258 9.33181C13.1271 9.19092 12.9765 9.20593 12.8057 9.37697L8.41701 13.7657Z" fill="#FF5F5F" />
                        </svg>
                        <svg onClick={() => { handleCardDelete(item) }} style={{ cursor: "pointer" }} width="25" height="24" viewBox="0 0 172 172" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <rect width="172" height="172" rx="15" fill="#FF8888" />
                          <path d="M85.9998 75.1115L124.111 37L135 47.8889L96.8886 86.0003L135 124.111L124.111 135L85.9998 96.8891L47.8889 135L37 124.111L75.1111 86.0003L37 47.8889L47.8889 37L85.9998 75.1115Z" fill="white" />
                        </svg>
                      </div>
            ...
            <h3 onClick={handleNewCard} className='m-4 underline-offset-2 decoration-2 cursor-pointer text-[#FF6A78] border-b-2 w-fit border-[#FF6A78]'>Add New Bank Details</h3>
          ...
          <Modal.Header className='bg-white  rounded-t-md' />
          {
            isOTPVerified ?
              <Modal.Body className="bg-white text-secondary  h-full xs:h-auto rounded-b-md">
                <div onClick={() => setIsOTPVerified(false)} className='cursor-pointer absolute top-4'>
                ...
                <div className='flex justify-between items-center gap-4 rounded-md p-4 border-[1px] mt-2'>
                  <input onChange={handleChangeReset} className='w-[100%] border-none outline-none' type={visible.oldPassword ? "text" : "password"} placeholder='Enter your old password' name="oldPassword" id="email" />
                  {
                    visible.oldPassword ?
                      <svg onClick={() => { setVisible({ ...visible, oldPassword: false }) }} width="24" height="21" viewBox="0 0 29 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      ...
                      :
                      <svg onClick={() => { setVisible({ ...visible, oldPassword: true }) }} width="24" height="21" viewBox="0 0 29 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className='flex justify-between items-center gap-4 rounded-md p-4 border-[1px] mt-2'>
                  <input onChange={handleChangeReset} className='w-[100%] border-none outline-none' type={visible.newPassword ? "text" : "password"} placeholder='Enter your new password' name="newPassword" id="email" />
                  {
                    visible.newPassword ?
                ...
                <div className='flex justify-between items-center gap-4 rounded-md p-4 border-[1px] mt-2'>
                  <input onChange={handleChangeReset} className='w-[100%] border-none outline-none' type={visible.confirmNewPassword ? "text" : "password"} placeholder='Enter your confirm password' name="newConfirmPassword" id="email" />
                  {
                ...
                <div onClick={handleSubmitReset} className='text-center bg-[#20124C] p-4 rounded-md flex items-center text-white gap-4 font-bold mt-4 cursor-pointer'>
                  <p className='text-center w-full text-xl font-medium tracking-wider'>Reset Password</p>
                </div>
              </Modal.Body> :
              <Modal.Body className="bg-white text-secondary  h-full xs:h-auto rounded-b-md">
                <div onClick={() => setIsResetPassword(false)} className='cursor-pointer absolute top-4'>
                  <svg width="10" height="16" viewBox="0 0 10 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M9.44033 14.0972C9.85557 14.5178 9.86807 15.213 9.46885 15.6503C9.26416 15.8743 8.99072 15.9872 8.71728 15.9872C8.45713 15.9872 8.19658 15.8854 7.99463 15.6806L1.21611 8.81616C1.01182 8.60964 0.895996 8.32353 0.895996 8.02529C0.895996 7.72601 1.01182 7.44052 1.21611 7.23339L7.99463 0.369592C8.40947 -0.0506281 9.06963 -0.0372585 9.46885 0.399622C9.86807 0.837531 9.85537 1.53276 9.44033 1.95256L3.44385 8.02529L9.44033 14.0972Z" fill="black" />
                  </svg>
                </div>
                ...
                <p className='mt-6 text-center'>Please enter the OTP which has sent your registered email address</p>
                ...
                <div className='text-center bg-[#20124C] p-4 rounded-md flex items-center text-white gap-4 font-bold mt-4 cursor-pointer'>
                  <p onClick={handleVerifyOTP} className='text-center w-full text-xl font-medium tracking-wider'>Verify</p>
                </div>
              </Modal.Body>
          }
        </div>
      ...
      >
        <Modal.Header>
          <h2 className='text-lg font-medium tracking-wider p-5' style={{ paddingLeft: "21px" }}>Bank Details</h2>
        </Modal.Header>
        <Modal.Body className="p-8 rounded-xl bg-white text-secondary  h-full rounded-b-md">
          <div key="Payment">
            ...
            <div className="App-payment">
              ...
              <form ref={formRef} onSubmit={handleSubmit}>
                ...
                <div onClick={handleSubmitCard} className="form-actions w-[100%] bg-blue-500 text-center mt-4 p-4 text-white rounded-xl cursor-pointer" style={{ backgroundColor: "rgb(45 35 74)" }}>
                  <button className="btn btn-primary btn-block text-center" >SUBMIT</button>
                </div>
              </form>
