...
const SignIn: React.FC<SignInProps> = ({ authenticationModal, setAuthenticationModal, userLoginData, setUserLoginData, onSuccess, onFailure }: any) => {
    ...
    //         try {
    //             const result = await getUserRegisteredWithGoogleData({
    //                 userId: decodedToken.sub,
    //                 name: decodedToken.name,
    //                 email: decodedToken.email,
    //                 role: userLoginData?.role,  // Dynamically get the role from state or selection
    //             });

    //             console.log("RESULT: ", result);

    //             if (result.success && isRegisterResponse(result)) {
    //                 let loginResult;

    //                 // If password is returned, use it to log the user in
    //                 if (result.password) {
    //                     loginResult = await loginUser({
    //                         emailOrUsername: decodedToken.email,
    //                         password: result.password,
    //                     });
    //                 } else {
    //                     // If no password, assume the user is already authenticated
    //                     loginResult = result;
    //                 }
    ...
    //                     return errorToast("Error logging in after signup: " + result.message);
    //                 }
    ...
    //                 const { accessToken, refreshToken, user } = loginResult.result;
    ...
    //                 });
    //             } else {
    //                 return errorToast("User not authenticated");
    //             }
    //         } catch (error) {
    //             console.error("Google SignUp Error:", error);
    //             errorToast("An error occurred during Google Sign Up");
    //         }
    //     }
    // };

    // function isRegisterResponse(result: any): result is RegisterResponse {
    //     return result && typeof result.success === 'boolean' && result.result && typeof result.result.user === 'object';
    // }
                ...
                <div className='rounded-md'>
                    ...
                    <Modal.Body className="bg-white text-secondary  h-full xs:h-auto rounded-b-md" style={{ fontFamily: "poppins, sans-serif" }}>
                        ...
                        <div className='cursor-pointer  m-auto rounded-md flex items-center justify-between  w-[250px] border-[1px] border-black'>
                            <div onClick={handleSignIn} className={`${authenticationModal.isSignInOpen && "bg-[#FF6A78]"} py-3 px-8 text-black`}>
                                Sign in
                            </div>
                            <div onClick={handleSignUp} className={`${authenticationModal.isSignUpOpen && "bg-[#FF6A78]"} py-3 px-8 text-black`}>
                                Sign up
                            </div>
                        </div>
                        <div className='mt-6'>
                        ...
                        <p onClick={handleForgotPassword} style={{ cursor: "pointer" }} className='text-right mt-2 text-[#EB4C60] font-medium'>Forgotten your Password?</p>
                        ...
                        <div className='flex items-center gap-6 justify-center mt-4' style={{ fontFamily: "poppins, sans-serif" }}>
                        ...
                        <div className='flex items-center gap-1 justify-center mt-4'>
                            ...
                            <span onClick={handleSignUp} className='cursor-pointer text-[#EB4C60] text-lg'>Sign Up</span>
                        </div>
