...
import { getDBData, getEntriesListingData, getWinnersListingData } from '../../Services/Admin/getDashboardData'
...
const AdminEntries = () => {
    ...
    const getDashboardData = async (page: number, search: string) => {
        try {
            const res = await getEntriesListingData(page, itemsPerPage, search);
    ...
    const handleDownload = async () => {
        try {
            const res = await getEntriesListingData(1, totalPages * itemsPerPage, searchQuery); // Fetch all data in one call
            const dataToDownload = res.result?.map((item: any) => ({
                Name: `${item.userFirstname} ${item.userLastname}`,
                "Raffle Name": item?.raffleName,
                'Number of Entries': `${item?.numberOfEntries || 0} `,
            ...
            XLSX.utils.book_append_sheet(workbook, worksheet, 'EntriesData');
            ...
            link.href = url;
            link.setAttribute('download', 'entries_data.xlsx');
    ...
    return (
        <div className='flex footer-manage ' style={{ fontFamily: "poppins, sans-serif" }}  >
            ...
            <div className='w-[100%] lg:w-[90%] mx-auto z-10 p-3 lg:p-10 duration-500'>
                                ...
                                <>
                                        ...
                                        <div className='w-[90%]'>
                                            <table className='w-[100%]' >
                                                <thead className='border-b-[2px] grid grid-cols-5 gap-4'>
                                                    ...
                                                    <th className='font-[400] text-[15px]'>Username</th>
                                                    <th className='font-[400] text-[15px]'>Raffle Entry</th>
                                                    <th className='font-[400] text-[15px]'>Number of Entries </th>
                                                    <th className='font-[400] text-[15px]'>Amount</th>
                                                    <th className='font-[400] text-[15px]'>Total</th>

                                                </thead>
                                                <tbody>
                                                    <tr className='grid grid-cols-5 gap-4'>
                                                        <td className='text-center'>{item?.userFirstname} {" "} {item?.userLastname}</td>
                                                        <td className='text-center'>{item?.raffleName}</td>
                                                        <td className='text-center'>{item?.numberOfEntries}</td>
                                                        <td className='text-center'>£ {item?.ticketPrice.toFixed(2) || "N/A"}</td>
                                                        <td className='text-center'>£ {(item?.ticketPrice * item?.numberOfEntries).toFixed(2) || "N/A"}</td>
                                    ...
                                    <div className='block lg:hidden flex lg:items-center justify-between bg-white p-2 rounded-md mt-4'>
                                        ...
                                        <div className='w-[100%]'>
                                            <table className='w-[100%]' >
                                                <thead className='border-b-[2px]'>
                                                    <th className='font-[400] text-[10px]'>Username</th>
                                                    {/* <th className='font-[400] text-[10px]'>Merchant</th> */}
                                                </thead>
                                                <tbody>
                                                    <tr>
                                                        <td className='text-center text-[10px]'>{item?.userFirstname} {" "} {item?.userLastname}</td>
                                                        {/* <td className='text-center text-[10px]'>{item?.ownerFirstname} {" "} {item?.ownerLastname}</td> */}
                                                    </tr>
                                                </tbody>
                                            </table>
                                            <br />
                                            <table className='w-[100%]' >
                                                <thead className='border-b-[2px]'>
                                                    <th className='font-[400] text-[10px]'>Raffle Entry</th>
                                                    {/* <th className='font-[400] text-[10px]'>Email</th> */}
                                                </thead>
                                                <tbody>
                                                    <tr>
                                                        <td className='text-center  text-[10px]'>{item?.raffleName}</td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                            <br />
                                            <table className='w-[100%]' >
                                                <thead className='border-b-[2px]'>
                                                    <th className='font-[400] text-[10px]'>Number of Entries</th>
                                                </thead>
                                                <tbody>
                                                    <tr>
                                                        <td className='text-center text-[10px]'>{item?.numberOfEntries}</td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                            <br />
                                            <table className='w-[100%]' >
                                                <thead className='border-b-[2px]'>
                                                    <th className='font-[400] text-[10px]'>Amount</th>
                                                    <th className='font-[400] text-[10px]'>Total</th>
                                                </thead>
                                                <tbody>
                                                    <tr>
                                                        <td className='text-center  text-[10px]'>£ {item?.ticketPrice || "N/A"}</td>
                                                        <td className='text-center  text-[10px]'>£ {item?.ticketPrice * item?.numberOfEntries || "N/A"}</td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
                                        {/* <Dropdown inline label="" dismissOnClick={true} renderTrigger={() =>
                                            ...
                                            <Dropdown.Item onClick={() => navigate(`/admin/entries/detail/${item._id}`, {
                                                state: {
                                                    item: item
                                                }
                                            })}>View details</Dropdown.Item>
