import React, { useEffect, useState } from 'react';
import AdminSidebar from '../../Components/Navbar/AdminSidebar';
import { Link, useNavigate } from 'react-router-dom';
import { useSearchParams } from "react-router-dom";
import { getWinnersList } from '@/Services/Owner/getPrize';
import { Dropdown } from 'flowbite-react'
import * as XLSX from 'xlsx';

const AdminWinners: React.FC = () => {
    const [searchParams, setSearchParams] = useSearchParams();
    const navigate = useNavigate();

    // Ensure the page is always a valid number
    const initialPage = parseInt(searchParams.get("page") || "1", 10) || 1;

    const [winnersData, setWinnersData] = useState<any[]>([]);
    console.log("winnersData", winnersData,)
    const [currentPage, setCurrentPage] = useState<number>(initialPage);
    const [totalPages, setTotalPages] = useState<number>(1);
    const [searchTerm, setSearchTerm] = useState<string>(""); // For customer name search
    const [startDate, setStartDate] = useState<string>('');
    const [endDate, setEndDate] = useState<string>('');
    const [isDownload, setIsDownload] = useState(false);
    const itemsPerPage = 10;

    useEffect(() => {
        getWinnersList(currentPage, itemsPerPage, searchTerm, isDownload, startDate, endDate)
            .then(res => {
                console.log("Fetched Winners Data:", res);
                setWinnersData(res.result || []);
                setTotalPages(res.totalPages || 1);
            })
            .catch(err => {
                console.error("Error fetching winners:", err);
            });
    }, [currentPage, searchTerm, startDate, endDate, isDownload]);



    const handlePageChange = (newPage: number) => {
        setCurrentPage(newPage);
        setSearchParams({ page: newPage.toString() });
    };


    const handlePrevPage = () => {
        if (currentPage > 1) {
            handlePageChange(currentPage - 1);
        }
    };

    const handleNextPage = () => {
        if (currentPage < totalPages) {
            handlePageChange(currentPage + 1);
        }
    };

    const handleDelete = async (item: any) => {
        navigate(`/admin/user/detail/${item.userId}`)
    };

    const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
        setSearchTerm(e.target.value);
        setCurrentPage(1); // Reset to the first page when searching
    };


    // Handling start date filter change
    const handleStartDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        setStartDate(e.target.value);
        setCurrentPage(1); // Reset to the first page when filtering
    };

    // Handling end date filter change
    const handleEndDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        setEndDate(e.target.value);
        setCurrentPage(1); // Reset to the first page when filtering
    };

    const handleDownloadClick = async () => {
        setIsDownload(true);
        let allData: any[] = [];
        let currentPage = 1;
        const limit = 10;

        try {
            // Loop to fetch all pages
            while (true) {
                const res = await getWinnersList(currentPage, limit, searchTerm, true, startDate, endDate);
                allData = [...allData, ...res.result];

                if (currentPage >= res.totalPages) {
                    break;
                }

                currentPage++;
            }

            // Flatten and map over all the prizes for each winner
            const data = allData.flatMap((winner: any) =>
                winner.main_prizes.map((prize: any) => ({
                    RaffleName: winner.raffle_name ?? "", // Show blank if not available
                    PrizeName: prize.prize_name ?? "",
                    WinnerName: `${prize.user?.firstname ?? ""} ${prize.user?.lastname ?? ""}`.trim(), // Trim to avoid extra space
                    Email: prize.user?.email ?? "",
                    Phone: prize.user?.phone ?? ""
                }))
            );

            const ws = XLSX.utils.json_to_sheet(data);
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, "Winners");
            XLSX.writeFile(wb, "winners.xlsx");

        } catch (error) {
            console.error("Download Error:", error);
        } finally {
            setIsDownload(false);
        }
    };



    return (
        <div className='flex footer-manage' style={{ fontFamily: "poppins, sans-serif" }}>
            <div className='hidden lg:block'>
                <AdminSidebar />
            </div>

            <div className='w-[100%] lg:w-[95%] mx-auto z-10 p-3 lg:p-10 duration-500'>
                <div className='m-auto w-[100%] lg:w-[90%]'>
                    <nav aria-label="breadcrumb" className="breadcrumb-container">
                        <ol className="breadcrumb">
                            <li className="breadcrumb-item">
                                <Link to="/admin/reports" className="breadcrumb-link">Dashboard</Link>
                            </li>
                            <li className='px-2'>{">"}</li>
                            <li className="breadcrumb-item active" aria-current="page">Winners</li>
                        </ol>
                    </nav>

                    <div className='w-[100%] lg:w-[100%] m-auto bg-[#F9F0F0] mt-4 p-4 rounded-md'>
                        <div className="flex items-center gap-4 w-full">
                            <h4 className="text-lg font-semibold w-full md:w-1/6">Winners</h4>
                            <div className="flex items-center gap-4 flex-wrap w-full">
                                <input
                                    type="text"
                                    className="border p-2 rounded-md outline-none w-full md:w-48"
                                    placeholder="Search"
                                    value={searchTerm}
                                    onChange={handleSearch}
                                />
                                <input
                                    className="border p-2 rounded-md outline-none cursor-pointer w-full md:w-48"
                                    type="date"
                                    value={startDate}
                                    onChange={handleStartDateChange}
                                    placeholder="From date"
                                />
                                <input
                                    className="border p-2 rounded-md outline-none cursor-pointer w-full md:w-48"
                                    type="date"
                                    value={endDate}
                                    onChange={handleEndDateChange}
                                    placeholder="To date"
                                />


                                {/* Download Button */}
                                <div
                                    className="p-3 bg-white rounded-full w-fit flex items-center justify-center cursor-pointer lg:ml-4"
                                    onClick={handleDownloadClick}
                                >
                                    <svg
                                        width="20"
                                        height="18"
                                        viewBox="0 0 20 18"
                                        fill="none"
                                        xmlns="http://www.w3.org/2000/svg"
                                    >
                                        <path
                                            d="M2 16H18V9H20V17C20 17.5523 19.5523 18 19 18H1C0.44772 18 0 17.5523 0 17V9H2V16ZM12 6H17L10 13L3 6H8V0H12V6Z"
                                            fill="black"
                                        />
                                    </svg>
                                </div>
                            </div>
                        </div>

                        {winnersData?.map((contact: any, i: number) => (
                            <>
                                <div className='hidden lg:block lg:flex items-center justify-between bg-white p-2 rounded-md mt-4'>
                                    <div className='w-[100%]'>
                                        <table className='w-full table-auto border-collapse'>
                                            <thead className='border-b-[2px]  grid-cols-5 gap-4'>
                                                <tr>
                                                    <td colSpan={6} className='text-center font-bold text-xl py-4' style={{ color: "#3d3d3d" }}>{contact.raffle_name}</td>
                                                </tr>
                                                <th className='font-[400] text-[15px]'>Name</th>
                                                <th className='font-[400] text-[15px]'>Email</th>
                                                <th className='font-[400] text-[15px]'>Phone number</th>
                                                <th className='font-[400] text-[15px]'>Prize Name</th>
                                                <th className='font-[400] text-[15px]'>Prize Value</th>
                                                <th className='font-[400] text-[15px]'>Action</th> {/* Added Action column for View Details */}
                                            </thead>
                                            <tbody>
                                                {contact?.main_prizes?.map((prize: any, index: number) => (
                                                    <>

                                                        <tr className=' grid-cols-5 gap-4 items-center' key={prize._id}>

                                                            <td className='text-center'>{prize?.user?.firstname} {prize?.user?.lastname}</td>
                                                            <td className='text-center'>{prize?.user?.email}</td>
                                                            <td className='text-center'>{prize?.user?.phone}</td>
                                                            <td className='text-center'>{prize?.prize_name}</td>
                                                            <td className='text-center'>{prize?.prize_value}</td>

                                                            {/* Action column */}
                                                            <td className='text-center'>
                                                                <Dropdown inline label="" dismissOnClick={true} renderTrigger={() => (
                                                                    <div className='flex items-center justify-center'>
                                                                        <svg
                                                                            className='p-2 bg-white view-icon'
                                                                            width="39"
                                                                            height="33"
                                                                            viewBox="0 0 19 13"
                                                                            fill="none"
                                                                            xmlns="http://www.w3.org/2000/svg"
                                                                            style={{
                                                                                cursor: "pointer",
                                                                                boxShadow: "2px 2px 5px #ccc",
                                                                                borderRadius: "5px",
                                                                            }}
                                                                        >
                                                                            <g clipPath="url(#clip0_174_422)">
                                                                                <path
                                                                                    fillRule="evenodd"
                                                                                    clipRule="evenodd"
                                                                                    d="M9.33217 0.693359C5.32099 0.693359 1.89544 3.06225 0.507568 6.40612C1.89544 9.74999 5.32099 12.1189 9.33217 12.1189C13.3434 12.1189 16.7689 9.74999 18.1568 6.40612C16.7689 3.06225 13.3434 0.693359 9.33217 0.693359ZM9.33217 10.2146C7.118 10.2146 5.32099 8.50842 5.32099 6.40612C5.32099 4.30383 7.118 2.59761 9.33217 2.59761C11.5463 2.59761 13.3434 4.30383 13.3434 6.40612C13.3434 8.50842 11.5463 10.2146 9.33217 10.2146ZM9.33217 4.12102C8.00046 4.12102 6.92546 5.1417 6.92546 6.40612C6.92546 7.67055 8.00046 8.69123 9.33217 8.69123C10.6639 8.69123 11.7389 7.67055 11.7389 6.40612C11.7389 5.1417 10.6639 4.12102 9.33217 4.12102Z"
                                                                                    fill="#A4A4A4"
                                                                                />
                                                                            </g>
                                                                            <defs>
                                                                                <clipPath id="clip0_174_422">
                                                                                    <rect width="17.6492" height="12.1872" fill="white" transform="translate(0.507568 0.3125)" />
                                                                                </clipPath>
                                                                            </defs>
                                                                        </svg>
                                                                    </div>
                                                                )}>
                                                                    <Dropdown.Item onClick={() => handleDelete(prize)}>View Details</Dropdown.Item>
                                                                </Dropdown>
                                                            </td>
                                                        </tr>
                                                    </>
                                                ))}
                                            </tbody>
                                        </table>
                                    </div>
                                </div>

                                {/* Mobile view */}
                                <div className='block lg:hidden flex lg:items-center justify-between bg-white p-4 rounded-md mt-4'>
                                    <div className='w-[100%]'>
                                        {contact?.main_prizes?.map((prize: any) => (
                                            <div className="bg-gray-50 p-4 rounded-lg shadow-md mb-4 relative">


                                                {/* User Information */}
                                                <div className="mb-2">
                                                    <table className='w-full'>


                                                        <tbody>
                                                            <tr>
                                                                <td colSpan={6} className='text-center font-bold text-xl py-4' style={{ color: "#3d3d3d" }}>{contact.raffle_name}</td>
                                                            </tr>
                                                        </tbody>
                                                    </table>
                                                </div>
                                                <div className="mb-2">
                                                    <table className='w-full'>

                                                        <thead className='border-b-[2px]'>
                                                            <tr>
                                                                <th className='font-[400] text-[12px] text-gray-700'>Name</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                            <tr>
                                                                <td className='text-center text-[12px]'>{prize?.user?.firstname} {prize?.user?.lastname}</td>
                                                            </tr>
                                                        </tbody>
                                                    </table>
                                                </div>

                                                <div className="mb-2">
                                                    <table className='w-full'>
                                                        <thead className='border-b-[2px]'>
                                                            <tr>
                                                                <th className='font-[400] text-[12px] text-gray-700'>Email</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                            <tr>
                                                                <td className='text-center text-[12px]'>{prize?.user?.email}</td>
                                                            </tr>
                                                        </tbody>
                                                    </table>
                                                </div>

                                                <div className="mb-2">
                                                    <table className='w-full'>
                                                        <thead className='border-b-[2px]'>
                                                            <tr>
                                                                <th className='font-[400] text-[12px] text-gray-700'>Prize Name</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                            <tr>
                                                                <td className='text-center text-[12px]'>{prize?.prize_name}</td>
                                                            </tr>
                                                        </tbody>
                                                    </table>
                                                </div>

                                                <div className="mb-2">
                                                    <table className='w-full'>
                                                        <thead className='border-b-[2px]'>
                                                            <tr>
                                                                <th className='font-[400] text-[12px] text-gray-700'>Phone Number</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                            <tr>
                                                                <td className='text-center text-[12px]'>{prize?.user?.phone}</td>
                                                            </tr>
                                                        </tbody>
                                                    </table>
                                                </div>
                                                {/* View Details Icon - Positioned at the top-right corner */}
                                                <div className="absolute right-2 bottom-5">
                                                    <Dropdown inline label="" dismissOnClick={true} renderTrigger={() => (
                                                        <div className='flex items-center justify-center cursor-pointer'>
                                                            <svg
                                                                className='p-2 bg-white view-icon'
                                                                width="39"
                                                                height="33"
                                                                viewBox="0 0 19 13"
                                                                fill="none"
                                                                xmlns="http://www.w3.org/2000/svg"
                                                                style={{
                                                                    cursor: "pointer",
                                                                    boxShadow: "2px 2px 5px #ccc",
                                                                    borderRadius: "5px",
                                                                    transition: "transform 0.2s",
                                                                }}
                                                            >
                                                                <g clipPath="url(#clip0_174_422)">
                                                                    <path
                                                                        fillRule="evenodd"
                                                                        clipRule="evenodd"
                                                                        d="M9.33217 0.693359C5.32099 0.693359 1.89544 3.06225 0.507568 6.40612C1.89544 9.74999 5.32099 12.1189 9.33217 12.1189C13.3434 12.1189 16.7689 9.74999 18.1568 6.40612C16.7689 3.06225 13.3434 0.693359 9.33217 0.693359ZM9.33217 10.2146C7.118 10.2146 5.32099 8.50842 5.32099 6.40612C5.32099 4.30383 7.118 2.59761 9.33217 2.59761C11.5463 2.59761 13.3434 4.30383 13.3434 6.40612C13.3434 8.50842 11.5463 10.2146 9.33217 10.2146ZM9.33217 4.12102C8.00046 4.12102 6.92546 5.1417 6.92546 6.40612C6.92546 7.67055 8.00046 8.69123 9.33217 8.69123C10.6639 8.69123 11.7389 7.67055 11.7389 6.40612C11.7389 5.1417 10.6639 4.12102 9.33217 4.12102Z"
                                                                        fill="#A4A4A4"
                                                                    />
                                                                </g>
                                                                <defs>
                                                                    <clipPath id="clip0_174_422">
                                                                        <rect width="17.6492" height="12.1872" fill="white" transform="translate(0.507568 0.3125)" />
                                                                    </clipPath>
                                                                </defs>
                                                            </svg>
                                                        </div>
                                                    )}>
                                                        <Dropdown.Item onClick={() => handleDelete(prize)}>View Details</Dropdown.Item>
                                                    </Dropdown>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                </div>


                            </>
                        ))}



                        {/* Pagination */}
                        <div className='text-xs lg:text-md text-right mt-4 flex items-center justify-end gap-2 cursor-pointer'>
                            <button onClick={handlePrevPage} disabled={currentPage === 1}>
                                <svg width="7" height="11" viewBox="0 0 7 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M1.5415 9.79541L5.78052 5.5564L1.5415 1.31738" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                                </svg>
                            </button>
                            <span>{currentPage} of {totalPages}</span>
                            <button onClick={handleNextPage} disabled={currentPage === totalPages}>
                                <svg width="7" height="11" viewBox="0 0 7 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M5.78052 9.79541L1.5415 5.5564L5.78052 1.31738" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                                </svg>
                            </button>
                        </div>

                    </div>
                </div>
            </div>
        </div>
    );
};

export default AdminWinners;
