...
const AdminUsers = () => {
    ...
    const [usersDataSuspend, setUsersDataSuspend] = useState<any>([]);
    ...
    const getDashboardData = async (page: number, search: string, isDownload: boolean = false) => {
        try {
            const res = await getDBData('users', page, itemsPerPage, search, false, isDownload);
    ...
    const getDashboardDatas = async (page: number, search: string, isDownload: boolean = false) => {
        try {
            const res = await getDBData('users', page, itemsPerPage, search, true, isDownload);
            setUsersDataSuspend(res.result);
    ...
    const handleDownload = async () => {
        try {
            ...
            const res = await getDBData('users', 1, totalPages * itemsPerPage, searchQuery, isSuspended, true); // Fetch all data in one call
            const dataToDownload = res.result.map((item: any) => ({
                Name: `${item?.firstname} ${item?.lastname}`,
                Email: item.email,
                "opt-in status": item.opted || false,
                    ...
                    <Tabs onActiveTabChange={handleTabChange}>
                                    ...
                                    usersData?.map((item: any, i: number) => (
                                        <>
                                                ...
                                                <div className='w-[80%]'>
                                                    <table className='w-[100%]' >
                                                        <thead className='border-b-[2px] grid grid-cols-4 gap-4'>
                                                            ...
                                                            <th className='font-[400] text-[15px]'>opt-in status</th>
                                                        ...
                                                        <tbody>
                                                            <tr className='grid grid-cols-4 gap-4'>
                                                                ...
                                                                <td className='text-center'>{item?.opted !== undefined ? String(item.opted) : 'false'}</td>
                                                ...
                                                <Dropdown inline label="" dismissOnClick={true} renderTrigger={() =>
                                                    <div className='flex items-center gap-6'>
                                                        <div className='flex items-center gap-3'>
                                                            <svg className='p-2 bg-white' width="39" height="33" viewBox="0 0 19 13" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ cursor: "pointer", boxShadow: "2px 2px 5px #ccc", borderRadius: "5px" }}
                                                            >
                                                    ...
                                                    <Dropdown.Item onClick={() => navigate(`/admin/user/transactions/${item._id}`, {
                                                        state: {
                                                            item: item
                                                        }
                                                    })}>View Transactions</Dropdown.Item>
                                                    <Dropdown.Item onClick={() => suspendUser(item._id)}>Archive Customer</Dropdown.Item>
                                                    <Dropdown.Item
                                                        onClick={() => navigate(`/admin/user/reset-password/${item._id}`, {
                                                            state: {
                                                                item: item
                                                            }
                                            ...
                                            <div className='block lg:hidden flex  justify-between bg-white p-2 rounded-md mt-4'>
                                                ...
                                                <div className='w-[100%]'>
                                                    <table className='w-[100%]' >
                                                        <thead className='border-b-[2px]'>
                                                            ...
                                                            <th className='font-[400] text-[10px]'>opt-in status</th>
                                                        </thead>
                                                        <tbody>
                                                            <tr>
                                                                <td className='text-center text-[10px]'>{`${item?.firstname} ${item?.lastname}`}</td>

                                                                <td className='text-center text-[10px]'>{item?.opted !== undefined ? String(item.opted) : 'false'}</td>
                                                            </tr>
                                                        </tbody>
                                                    </table>
                                                    <br />
                                                ...
                                                <Dropdown inline label="" dismissOnClick={true} placement="left" renderTrigger={() =>
                                                    <div className='flex items-center gap-6'>
                                                        <div className='flex items-center gap-3'>
                                                            <svg className='p-2 bg-white user-view-icon' width="39" height="33" viewBox="0 0 19 13" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ cursor: "pointer", boxShadow: "2px 2px 5px #ccc", borderRadius: "5px" }}
                        ...
                        <Tabs.Item title='Archive'>
                            <div>
                                ...
                                {
                                    usersDataSuspend?.map((item: any, i: number) => (
                                        <>
                                            <div className='hidden lg:block lg:flex items-center justify-between bg-white p-2 rounded-md mt-4'>
                                                ...
                                                <Dropdown inline label="" dismissOnClick={true} renderTrigger={() =>
                                                    ...
                                                    <Dropdown.Item onClick={() => unsuspendUser(item._id)}>Restore Customer</Dropdown.Item>
